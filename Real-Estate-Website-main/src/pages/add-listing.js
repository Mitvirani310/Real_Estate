...
const updaterent = async (e) => {
  e.preventDefault();
  if (!id || !customername) return;
  axios.put('http://localhost:8080/rentproperty/'+id, {
      id:id,
      customername:customername,
      mobileno:mobileno,
      email: email,
...
const deletesold = async (e) => {
  e.preventDefault();
  if (!id) return;
  axios.delete('http://localhost:8080/soldproperty/'+id)
  .then((res) => {
      alert("SuccessFully Complete The Task!")  
  }).catch((err) => {
      alert(err);
...
const deleterent = async (e) => {
  e.preventDefault();
  if (!id) return;
  axios.delete('http://localhost:8080/rentproperty/'+id)
  .then((res) => {
      alert("SuccessFully Complete The Task!")  
  }).catch((err) => {
      alert(err);
